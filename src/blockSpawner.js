...
function blockSpawner() {
    ...
    for (i; i < lowestBlock + 60; i++) {
        ...
        if (i >= blocks.length) {
            ...
            // Détermination de la position verticale du bloc en fonction du type de bloc précédent et de la présence d'oiseau ou d'alien
            ...
            } else if (blocks[i].obstacle !== 0 || blocks[i].particule !== 0 || blocks[i].alien !== 0) {
                blocks[i].y = (blocks[i - 1].y) - ((Math.random() * (5 + (difficulty * 25))) + 50); // Positionnement pour les blocs avec oiseau, particule ou alien
            } else if (blocks[i - 1].obstacle !== 0 || blocks[i - 1].particule !== 0 || blocks[i - 1].alien !== 0) {
                blocks[i].y = (blocks[i - 1].y) - ((Math.random() * (5 + (difficulty * 25))) + 50); // Positionnement pour les blocs avec oiseau, particule ou alien précédent
            } else {
